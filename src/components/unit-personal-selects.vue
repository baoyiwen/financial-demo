  ...
  export default {
    ...
    methods: {
      ...
      treeLoadNode: function (node, resolve) {
        ...
        if(!that.isFirst){
          orgCode = node.data.organizationCode
        }
        that.$http.get(Config.portalApiUrl + `/organization/orgTree/${orgCode}?isFirst=${that.isFirst}`).then(function (data) {
      ...
      getPersonalTable: function (val) {
        ...
        params.organizationCode = that.orgCode
